...
**为什么支付要拆成前后端两半？** 和普通网页调第三方支付（支付宝/Stripe）是同一套模型：**任何涉及金额与密钥的计算都不能放前端**。前端代码用户可见、可改，一旦把商户密钥（API Key）放进小程序，别人就能伪造订单、篡改金额。所以微信支付的分工是固定的——**前端拉起，后端下单 + 签名 + 验签 + 处理回调**。这条和第 [11-login-auth](../11-login-auth/) 里"敏感逻辑放后端"的原则完全一致。
...
- 🔗 上一步登录拿 openid → [11-login-auth](../11-login-auth/)；网络请求基础 → [09-network-storage](../09-network-storage/)；官方文档 → <https://pay.weixin.qq.com/doc/v3/merchant/4012791856>。
